import { getAgeInYears, extractContactFields, scoreLead, inferStateFromKeyword } from "@/lib/lead-utils";
...
export function transformCandidateToLead(candidate: YouTubeChannelCandidate, searchKeyword: string): LeadRecord {
  ...
  const state = inferStateFromKeyword(searchKeyword);
  return {
...
export function applyLeadFilters(leads: LeadRecord[], filters: LeadFilters): LeadRecord[] {
  ...
  function countryMatches(leadCountry: string, filterCountry: string): boolean {
    const lc = leadCountry.trim().toLowerCase();
    const fc = filterCountry.trim().toLowerCase();
    if (lc === fc) return true;
    const codes = countryCodeMap[fc];
    if (codes && codes.includes(lc)) return true;
    return false;
  }
  ...
  const normalized = leads.filter((lead) => {
    ...
    if (filters.country && !countryMatches(lead.country, filters.country)) return false;
