    ...
    handlers: {
      GET: async () => {
        try {
          ...
          const res = await fetch(`${base.replace(/\/$/, "")}/api/lead-sheets`, {
            headers,
            cache: "no-store",
          });
          if (!res.ok) {
            ...
            return new Response(
              JSON.stringify({ error: "Lead Finder unreachable", details: text.slice(0, 500) }),
          ...
          return new Response(JSON.stringify({ error: "Failed to fetch Lead Sheets" }), {
            status: 500,
            headers: { "content-type": "application/json" },
          });
        }
