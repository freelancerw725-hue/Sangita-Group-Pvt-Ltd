      ...
      GET: async ({ request }) => {
        try {
          ...
          if (idParam) {
            ...
            if (!progress) {
              return new Response(
                JSON.stringify({ error: "Campaign not found or Bulk Mail unreachable" }),
          ...
          return new Response(JSON.stringify({ campaigns: list }), {
            headers: { "content-type": "application/json" },
          });
        } catch (e) {
          console.error("BULK_MAIL_PROGRESS_PROXY_ERROR", e);
          return new Response(JSON.stringify({ error: "Failed to fetch Bulk Mail progress" }), {
            status: 500,
            headers: { "content-type": "application/json" },
          });
        }
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!["pause", "resume", "cancel", "start"].includes(action)) {
            return new Response(
              JSON.stringify({ error: "Invalid action. Use pause|resume|cancel|start" }),
          ...
          if (!ok) {
            return new Response(JSON.stringify({ error: `Failed to ${action} campaign` }), {
              status: 502,
              headers: { "content-type": "application/json" },
            });
          }
          ...
          const progress = await fetchBulkMailProgress(id);
          return new Response(JSON.stringify({ success: true, action, progress }), {
            headers: { "content-type": "application/json" },
          });
        } catch (e) {
          console.error("BULK_MAIL_CONTROL_ERROR", e);
          return new Response(JSON.stringify({ error: "Failed to control campaign" }), {
            status: 500,
            headers: { "content-type": "application/json" },
          });
        }
