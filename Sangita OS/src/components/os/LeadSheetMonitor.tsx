    ...
    <div className="rounded-xl border border-border bg-card p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            Manual approval + template + sendAt = automatic sending (no manual START needed at send
            time)
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="py-6 text-center text-sm text-muted-foreground">
          No Lead Sheets yet. Approve leads in Lead Finder → create sheet → select template → set
          send time.
        </div>
      ) : (
            ...
            <div key={s.id} className="rounded-xl border border-border bg-background p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Approved: {s.approvedLeads} / {s.totalLeads} · Valid:{" "}
                    {s.verificationSummary.valid} · Invalid: {s.verificationSummary.invalid}
                  </div>
                </div>
                <span
                  className={`text-[10px] px-2 py-1 rounded-full border font-medium uppercase tracking-widest ${statusColor(s.status)}`}
                >
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                <div className="rounded-md border border-border bg-card p-2">
                  <div className="text-muted-foreground flex items-center gap-1">
                    <CheckCircle className="h-3 w-3" /> Template
                  </div>
                  <div className="font-medium mt-1">
                    {s.templateName ?? <span className="text-amber-400">Not selected</span>}
                  </div>
                </div>
                <div className="rounded-md border border-border bg-card p-2">
                  <div className="text-muted-foreground flex items-center gap-1">
                    <Calendar className="h-3 w-3" /> Send Time
                  </div>
                  <div className="font-medium mt-1">
                    {s.sendAt ? (
                      new Date(s.sendAt).toLocaleString()
                    ) : (
                      <span className="text-amber-400">Not scheduled</span>
                    )}
                  </div>
                </div>
                <div className="rounded-md border border-border bg-card p-2">
                  <div className="text-muted-foreground flex items-center gap-1">
                    <Clock className="h-3 w-3" /> Campaign
                  </div>
                  <div className="font-medium mt-1">
                    {s.scheduledCampaignId ? `Campaign #${s.scheduledCampaignId}` : "Not created"}
                  </div>
                </div>
              </div>
              {s.status === "scheduled" && s.sendAt && new Date(s.sendAt) > new Date() && (
                <div className="mt-2 text-xs text-muted-foreground flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Waiting until {new Date(s.sendAt).toLocaleString()}{" "}
                  — will auto-start and respect daily/hourly limits, pause/cancel respected
                </div>
              )}
              {s.status === "scheduled" && s.sendAt && new Date(s.sendAt) <= new Date() && (
                <div className="mt-2 text-xs text-emerald-400 flex items-center gap-1">
                  <Send className="h-3 w-3" /> Send time reached — n8n will re-check approval and
                  start campaign automatically (idempotent)
                </div>
              )}
              {s.status === "draft" && (
                <div className="mt-2 text-xs text-amber-400">
                  WAITING_FOR_APPROVAL — approve leads to make eligible (valid ≠ approved)
                </div>
              )}
            </div>
          ))}
