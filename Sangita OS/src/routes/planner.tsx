...
type Block = {
  ...
  title: string;
  type: "focus" | "meeting" | "call" | "break" | "admin";
  impact?: string;
  done?: boolean;
};
const INITIAL: Block[] = [
  ...
  { time: "13:00", end: "14:00", title: "Lunch", type: "break" },
  {
    ...
    title: "Product review — Libriofy v1.4",
    type: "meeting",
    impact: "+18% CVR",
  },
  {
    ...
    title: "Deep work — recruit VP Marketing",
    type: "focus",
    impact: "Hire",
  },
  {
    ...
    title: "Follow-ups (Nexora, Kestrel)",
    type: "admin",
    impact: "+₹5.2L",
  },
...
export const Route = createFileRoute("/planner")({
  head: () => ({
    meta: [
      { title: "Daily Planner — Sangita OS" },
      { name: "description", content: "AI-ranked daily plan with time-blocked focus." },
    ],
  }),
  ...
  return (
    <AppLayout>
      <div className="p-6 max-w-[1400px] mx-auto">
        <PageHeader
          eyebrow="Daily OS"
          title="Your day, ranked by impact"
          description="AI blocks your calendar around what moves revenue and reduces risk. Toggle blocks as done to keep the score honest."
          actions={
            <div className="flex gap-2">
              <Button size="sm" variant="outline" className="gap-2">
                <Sparkles className="h-3.5 w-3.5 text-primary" /> Re-plan with AI
              </Button>
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          ...
          <Kpi
            label="Meetings"
            value={String(blocks.filter((b) => b.type === "meeting").length)}
            sub="2 external"
          />
              ...
              {blocks.map((b, i) => {
                ...
                return (
                  <motion.div
                    key={i}
                    layout
                    className={`rounded-lg border ${m.color} p-3 flex items-center gap-3 ${b.done ? "opacity-50" : ""}`}
                  >
                    ...
                    <Checkbox
                      checked={!!b.done}
                      onCheckedChange={() =>
                        setBlocks((p) => p.map((x, j) => (j === i ? { ...x, done: !x.done } : x)))
                      }
                    />
                    <Icon className="h-4 w-4" />
                    <div className="flex-1">
                      <div className={`text-sm font-medium ${b.done ? "line-through" : ""}`}>
                        {b.title}
                      </div>
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
              <div className="text-sm font-medium text-primary flex items-center gap-2">
                <Sparkles className="h-4 w-4" /> AI daily briefing
              </div>
              <div className="text-xs text-muted-foreground mt-2">
                Top 3 revenue moves today: close Acme MSA (₹8.4L), ship Libriofy checkout (+18%
                CVR), follow up Nexora (₹3.2L).
              </div>
              <div className="text-xs text-muted-foreground mt-2">
                Risk: 2 overdue invoices. AI can auto-chase now.
              </div>
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="space-y-2 text-xs">
                <div className="rounded-md border border-border p-2">
                  <div className="font-medium">Rajat @ Acme — 11:00</div>
                  <div className="text-muted-foreground">
                    MSA v3, wants annual invoicing. Push 24-mo term at 8% discount.
                  </div>
                </div>
                <div className="rounded-md border border-border p-2">
                  <div className="font-medium">Product review — 14:00</div>
                  <div className="text-muted-foreground">
                    Checkout redesign on staging. Baseline CVR 2.1% → target 2.5%.
                  </div>
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <ul className="text-xs space-y-1 text-muted-foreground list-disc pl-4">
                <li>What moved revenue today?</li>
                <li>What blocked progress?</li>
