...
const patchSchema = z.object({
  leadStatus: z
    .enum(["New", "Contacted", "Replied", "Interested", "Closed", "Not Interested"])
    .optional(),
...
export async function PATCH(request: Request, context: { params: Promise<{ channelId: string }> }) {
  try {
    ...
    if (parsed.data.lastFollowupTime !== undefined)
      patch.lastFollowupTime = parsed.data.lastFollowupTime;
    ...
    if (parsed.data.meetingScheduled !== undefined)
      patch.meetingScheduled = parsed.data.meetingScheduled;
    ...
    if (parsed.data.lastEmailSubject !== undefined)
      patch.lastEmailSubject = parsed.data.lastEmailSubject;

    if (parsed.data.approved !== undefined && parsed.data.verified === undefined)
      patch.verified = parsed.data.approved;
    if (parsed.data.approved !== undefined && parsed.data.sendMail === undefined)
      patch.sendMail = parsed.data.approved;
