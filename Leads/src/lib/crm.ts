...
export function normalizeLeadRecord(lead: LeadRecord): LeadRecord {
  ...
  // Backward compatibility: infer state if not present
  const state = lead.state ?? (lead.searchKeyword ? inferStateFromKeyword(lead.searchKeyword) : "Other");
  ...
  return {
    ...
    // Phase 3 — verification & approval defaults (preserve existing, do not auto-approve)
    emailVerificationStatus: lead.emailVerificationStatus ?? "not_verified",
    verifiedAt: lead.verifiedAt ?? null,
    verificationProvider: lead.verificationProvider ?? null,
    verificationError: lead.verificationError ?? null,
    verificationScore: lead.verificationScore ?? null,
    approvalStatus: lead.approvalStatus ?? "pending_review",
