...
describe("Phase 3 — Email Verification + Approval + Sheets", () => {
  ...
  // 13. selected template reference is stored
  it("13. selected template reference stored on sheet", async () => {
  ...
  // 14. Bulk Mail is NOT triggered automatically
  it("14. Bulk Mail handoff is manual — sheet stays draft without template", async () => {
    const l = makeLead("UC_HND_1", "valid@example.com", { approvalStatus: "approved" });
    await upsertLeads([l]);
    if (hasDatabaseUrl()) await updateDbLead("UC_HND_1", { approvalStatus: "approved" } as never);
    else await updateLeadByChannelId("UC_HND_1", { approvalStatus: "approved" } as never);
    const sheet = await createLeadSheet({ name: "Handoff Test", leadIds: ["UC_HND_1"] });
    expect(sheet.status).toBe("draft");
    expect(sheet.templateId).toBeNull();
    // Without template, handoff should fail (we simulate check)
    ...
    // Here we just verify sheet does not automatically trigger sending
