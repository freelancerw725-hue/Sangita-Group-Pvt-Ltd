...
function getBulkMailConfig() {
  const base =
    process.env.BULK_MAIL_BASE_URL?.trim() ||
    process.env.BULK_MAIL_URL?.trim() ||
    process.env.APP_URL?.trim() ||
    "https://sangita-email-outreach.vercel.app";
  const key =
    process.env.BULK_MAIL_API_KEY?.trim() ||
    process.env.BULK_MAIL_CAMPAIGN_KEY?.trim() ||
    process.env.BULK_MAIL_IMPORT_KEY?.trim() ||
    "";
  return { base: base.replace(/\/$/, ""), key };
}

export async function fetchBulkMailCampaigns(): Promise<
...
export async function fetchBulkMailProgress(campaignId: number): Promise<BulkMailProgress | null> {
  ...
  try {
    const headers: Record<string, string> = {};
    if (key) headers["x-api-key"] = key;
    const res = await fetch(`${base}/api/campaigns/${campaignId}/progress`, {
      headers,
      cache: "no-store",
    });
...
export async function controlBulkMailCampaign(
  campaignId: number,
